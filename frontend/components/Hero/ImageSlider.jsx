...
const ImageSlider = ({ banners }) => {
  ...
  return (
    <div className="relative">
      {/* Left Arrow */}
      {showArrows && (
        <div className="absolute left-2 top-1/2 transform -translate-y-1/2 cursor-pointer rounded-full bg-white z-50">
          <svg
      ...
      <div
        className="flex gap-3 overflow-x-auto hide-scrollbar scrollable-container"
        ref={scrollableRef}
      >
        {banners.map((banner, index) => (
          <button
            key={index}
            className="bg-white shadow-lg px-1 flex flex-col justify-center items-center max-h-32 mb-2 min-w-[8rem] max-w-[8rem]"
          >
